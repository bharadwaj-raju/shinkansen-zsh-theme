...
# DIR
if [ ! -n "${SHINKANSEN_DIR_SHOW+1}" ]; then
  SHINKANSEN_DIR_SHOW=true
fi
...
# CONTEXT
if [ ! -n "${SHINKANSEN_CONTEXT_SHOW+1}" ]; then
  SHINKANSEN_CONTEXT_SHOW=false
fi
...
prompt_context() {
  [[ $SHINKANSEN_CONTEXT_SHOW == false ]] && return
...
prompt_dir() {
  if [[ $SHINKANSEN_DIR_SHOW == false ]]; then
    return
  fi
...
prompt_time() {
  if [[ $SHINKANSEN_TIME_SHOW == false ]]; then
    return
  fi

  if [[ $SHINKANSEN_TIME_12HR == true ]]; then
    prompt_segment $SHINKANSEN_TIME_BG $SHINKANSEN_TIME_FG "$(date +%r)"
  else
    prompt_segment $SHINKANSEN_TIME_BG $SHINKANSEN_TIME_FG "$(date +%R)"
  fi
}
...
prompt_status() {
  if [[ $SHINKANSEN_STATUS_SHOW == false ]]; then
    return
  fi
